...
while frame_idx < num_frames:
    ...
    while not done and goal != np.argmax(state):
        ...
        if frame_idx % 6000 == 0:
            clear_output(False)
            n = 80 #mean reward of last 100 episodes
            plt.cla
            plt.figure(figsize=(10,5))
            plt.title(frame_idx)
            #plt.plot([np.max(all_rewards[i:i + n]) for i in range(0, len(all_rewards), n)])
            plt.plot([np.min(all_rewards[i:i + n]) for i in range(0, len(all_rewards), n)])
