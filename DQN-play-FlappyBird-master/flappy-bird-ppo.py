...
Transition = namedtuple('Transition', ['obs4', 'action', 'a_log_prob', 'reward', 'obs4_next','done'])
...
class PPO():
    ...
    def select_action(self,state):
        ...
        return action_button,action_onehot,action_prob[:,action_index.item()].item()
    ...
    def update(self,gamma):
        #state = torch.tensor([t.obs4 for t in self.buffer],dtype = torch.float)
        state_list = [t.obs4 for t in self.buffer]
        
        state = torch.stack(state_list)
        #action = torch.tensor([t.action for t in self.buffer], dtype = torch.long).view(-1,1)
        action = LongTensor([t.action for t in self.buffer]).view(-1,1)
        reward = [t.reward for t in self.buffer]
        old_action_log_prob = FloatTensor([t.a_log_prob for t in self.buffer]). view(-1,1)
        ...
        for _ in range(self.ppo_update_time):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.buffer))),self.batch_size,False):
                ...
                #c_state = list(map(lambda x,y:x[y],state,index))
                V = self.critic_net(state[index])
                #V = self.critic_net(c_state)
                delta = Gt_index - V
...
def main():
    ...
    while episode_num < 200000:
        prob = random.random()
        a = 0.9
        b = (100000 - max(0,time_step - observe))
        threshold = 0.1 + max(0,a*b / 100000)
        if time_step % FRAME_PER_ACTION ==0:
            if prob <= threshold:
                ...
                _,_,action_prob = agent.select_action(obs4)
            else:
                action_button, action_onehot, action_prob = agent.select_action(obs4)
