...
from new_agent import Camera_Agent
...
class StatusHandler(tornado.websocket.WebSocketHandler):
    ...
    def received_message(self, m):
        ...
        obs = {"image": image, "depth":depth, "ir":ir, "ground":ground, "compass":compass, "target":target}
        reward = np.array(dat['reward'], dtype=np.float32)
        
        end_episode = np.array(dat['endEpisode'], dtype=np.bool)

        actions = self.agent.select_action(obs,0.1)

        print(actions, reward)
        self.send_action(actions)
        next_obs =  {"image": image, "depth":depth, "ir":ir, "ground":ground, "compass":compass, "target":target}
        if args.model == 'None':
            #self.agent.store_experience(observation, actions, reward, end_episode)
            self.agent.learn(obs,next_obs,reward,end_episode,5000,32,1e-6,0.99,50,10,1)
