...
class StatusHandler(tornado.websocket.WebSocketHandler):
    ...
    def received_message(self, m):
        ...
        for i in range(agent_count):
            img = np.array(Image.open(io.BytesIO(bytearray(dat['image'][i]))).convert('L'),dtype = np.float32)
            image.append(img)
        ...
        if args.model == 'None':
            if self.agent.exploration_step <= 1000:
                self.agent.exploration_step += 1
                _,actions = self.agent.select_action(observation,1)
                self.agent.store_experience(observation,actions,reward,end_episode)
            else:
                _,actions = self.agent.select_action(observation,0.1)
                self.agent.store_experience(observation, actions, reward, end_episode)
                self.agent.learn()
